...
async function selectAccount(userId: string, sizeBytes: bigint, reservedBytesByAccount = new Map<string, bigint>()) {
  ...
  const mode = (['most_available', 'round_robin', 'priority'].includes(policy.mode)
    ? policy.mode
    : 'most_available') as RoutingMode
...
async function streamToBuffer(fileStream: NodeJS.ReadableStream) {
  const chunks: Buffer[] = []
  ...
  try {
    ...
    const uploadOne = async (
      ...
      try {
        ...
        const streamedBytes = BigInt(buffer.length)
        ...
        if (account.provider === 's3') {
          ...
          await uploadS3Object(config, providerFileId, Readable.from(buffer), meta.mimeType)
